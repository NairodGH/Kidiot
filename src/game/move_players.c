...
static bool move_players_baby_split(char **map, entity_t *play, int keys[])
{
    if (keys[4] == 1 && !(is_obstacle
        (map[(int)play->baby->pos.x - 1][(int)play->baby->pos.y + 1], play))) {
    ...
    entity_t *play, int keys[])
{
    ...
    if (keys[0] == 1 && !(is_obstacle
        (map[(int)play->baby->pos.x][(int)play->baby->pos.y + 1], play)))
        play->baby->pos.y += 1;
    if (keys[1] == 1 && !(is_obstacle
        (map[(int)play->baby->pos.x][(int)play->baby->pos.y - 1], play)))
        play->baby->pos.y -= 1;
    if (keys[2] == 1 && !(is_obstacle
        (map[(int)play->baby->pos.x - 1][(int)play->baby->pos.y], play)))
        play->baby->pos.x -= 1;
...
static bool move_players_mom_split(char **map, entity_t *play, int keys[])
{
    if (keys[13] == 1 && !(is_obstacle
        (map[(int)play->mom->pos.x - 1][(int)play->mom->pos.y + 1], play))) {
    ...
    entity_t *play, int keys[])
{
    ...
    if (keys[9] == 1 && !(is_obstacle
        (map[(int)play->mom->pos.x][(int)play->mom->pos.y + 1], play)))
        play->mom->pos.y += 1;
    if (keys[10] == 1 && !(is_obstacle
        (map[(int)play->mom->pos.x][(int)play->mom->pos.y - 1], play)))
        play->mom->pos.y -= 1;
    if (keys[11] == 1 && !(is_obstacle
        (map[(int)play->mom->pos.x - 1][(int)play->mom->pos.y], play)))
        play->mom->pos.x -= 1;
